...
async def accounts(page: Page) -> None:
    ...
    if await page.locator("div").filter(has_text=re.compile(fr"{o14}")).get_by_label("Sign out").is_visible():
        ...
        try:
            ...
            logging.info(f"Error verifying signed out: {e}")
   ...
   # If outlook is still in reconnect, then reconnect
    ...
    if decision:
        ...
        try:
            await page.locator("div").filter(has_text=re.compile(r"^Primaryrutvikqatest@outlook\.com$")).get_by_label("Sign out").click()
            logging.info("trying sign out of Outlook")
        except:
            await page.locator("div").filter(has_text=re.compile(r"^Primaryrutvik@bluemind\.com$")).get_by_label("Sign out").click()
    ...
    async with async_playwright() as p:
        ...
        try: 
            ...
            try:
                ...
                logging.info(f"Error stopping tracing: {e}")
